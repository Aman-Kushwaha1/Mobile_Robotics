    ...
    def get_possible_moves(self):
        ...
        for move in potential_moves:
            ...
            if str(move) not in self.explored and str(move) not in self.not_explored :
                self.not_explored[str(move)] = self.pos_depth +1    #updating depth values
...
#Shortcut way to find the path
print("--------------------------ttt")
print(path)
best_path_shortcut = np.zeros([len(grid), len(grid)], dtype=int)
hval = path[goal[0], goal[1]]
best_path_shortcut[goal[0], goal[1]] = 1
pm = bfs.generate_potential_moves(goal)
while hval>1:
    ...
    for m in pm:
        ...
        m1 = m
        hval1 = path[m1[0], m1[1]]
        if hval1 == hval-1:
            goal1 = m1
            hval = hval1

    pm = bfs.generate_potential_moves(goal1)
    best_path_shortcut[goal1[0], goal1[1]] = 1
print("\n Beat path Shortcut\n",best_path_shortcut)